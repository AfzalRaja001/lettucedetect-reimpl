...
- Works on any black-box LLM — no access to internal weights, logits, or probabilities needed
- The first systematic zero-resource hallucination detector; opened the field

**Disadvantages:**
- Requires generating multiple samples per answer (N× inference cost and latency)
...
**Its limitation:** Expensive at inference time, and consistency ≠ groundedness — a model can be consistently wrong.

---

### 2.2 RAGTruth (Niu et al. — ACL 2024)

Not a detector — the **foundational benchmark** nearly every supervised method below trains on.
...
**Advantages:**
- Far cheaper and faster than an LLM-judge (Luna reports ~97% cost and ~91% latency reduction vs. GPT-3.5-based evaluation)
...
- At 440M parameters, it is roughly 3× the size of LettuceDetect-base while scoring lower (65.4 vs. 76.1 example-level F1 on RAGTruth)
...
- Does not stop at detection — actually **mitigates** hallucination by improving the generator itself via DPO
...
**Disadvantages:**
- Requires **white-box access** to the generator's attention maps and FFN activations — cannot be used as a detector bolted in front of a closed-API model like GPT-4
...
- 150M parameters (base) — roughly **30× smaller** than the best LLM-based methods, at competitive accuracy
...
**Disadvantages / Limitations (see §6 for the full ledger):**
...
**Its limitation:** See §6 — mainly generalization, calibration, and monolinguality.
...
**Approach:** Directly extends LettuceDetect to Turkish: machine-translates RAGTruth (~17,790 instances) and fine-tunes three different encoder backbones, including a Turkish-specific ModernBERT.
...
**Why this matters for our project:** this is the *direct precedent* for "translate RAGTruth into another language and fine-tune" — worth knowing before proposing the same recipe for Hindi (see §7).
...
- Uses a clean train/test domain split (RAGTruth → SQuAD) to isolate genuine generalization from memorized artifacts
...
**Its limitation:** Diagnostic only — leaves "what do we do about it" open. **Our contribution (§7) targets this same class of problem, using PsiloQA as the concrete out-of-domain test rather than a hand-built one.**
...
**Disadvantages:**
- Labels are LLM-generated, not human-verified — inherits whatever blind spots GPT-4o has as an annotator
...
**Advantages:**
- Detection signal doubles as a training signal, which the authors show *also reduces* the generator's hallucination rate — detection and mitigation from one joint objective
- No separate inference pass needed at serving time (once trained, the generator self-reports)

**Disadvantages:**
- Requires training access to the generator itself — cannot be bolted onto a closed API model, same limitation class as ReDeEP
...
- Provides a concrete existence proof that calibration + abstention is practical and cheap to add to a hallucination pipeline

**Disadvantages:**
- Evaluated on HaluEval, not RAGTruth — not directly comparable to the LettuceDetect line of results
...
**Its limitation:** Different benchmark, so not a plug-and-compare with the RAGTruth leaderboard — but the *methodology* (calibrate + abstain) is directly reusable. **This is the direct inspiration for the calibration/ECE reporting folded into our contribution (§7).**
...
| **Turkish-RAGTruth** | Turk-LettuceDetect | Machine-translated version of RAGTruth, ~17,790 instances | Turkish | Released alongside the Turk-LettuceDetect paper |
...
| **PsiloQA** | PsiloQA paper | Wikipedia-derived QA pairs across 14 languages; 63,792 training examples total, ~23,000 of them English; 2,897 test examples across all languages; hallucinated answers elicited from multiple LLMs in a no-context setting, spans auto-annotated by GPT-4o | 14 languages | Public |
...
| **HaluEval** | HALT-RAG | ~35K samples across QA, dialogue, and summarization, each with a correct and a hallucinated LLM response, used as the standard general-purpose hallucination benchmark | English | Public |
...
Feb 2025   LettuceDetect  ★ (this project)
           └─ Swaps Luna's backbone for ModernBERT (8K context) — removes the
...
**The throughline:** each step reacts to a specific limitation of what came before —
SelfCheckGPT's cost problem → Luna's context-window problem → LettuceDetect's answer to that → then the field fans out into *generalization* (does it work outside RAGTruth?), *multilinguality* (does it work in other languages?), and *integration* (can detection be folded into the generator itself, or does it need its own model?).

---

## 5. Deep Dive: The LettuceDetect Paper

*(This section is the detailed walkthrough — use it for the technical portion of your presentation. It assumes no prior background beyond general ML.)*
...
- **LLM-as-judge** (prompt GPT-4 to check the answer): accurate but expensive — doubles or triples inference cost and adds latency at production scale.
- **Small encoder-based classifiers** (Luna and its predecessors): cheap, but built on BERT-family backbones capped at 512 tokens. Realistic RAG contexts run 2,000–8,000 tokens, so these detectors were **physically blind** to most of the evidence they were supposed to judge against.
...
**The 512-token ceiling:** Original BERT learned one position vector per slot from 1–512; slot 513 simply doesn't exist. **ModernBERT** (AnswerDotAI, Dec 2024) replaces this with **RoPE (Rotary Position Embeddings)**, which encode position as a mathematical rotation defined for any position — removing the hard ceiling and extending capacity to **8,192 tokens**. This is the paper's single key technical enabler: the idea of token-level hallucination classification wasn't new, it simply wasn't *possible* with a small model before a long-context encoder existed.
...
- **Example-level F1:** collapse token predictions to one label per example (any flagged token → example flagged), compute precision/recall/F1 over examples. Answers "does this response need review?"
...
- Cuts inference cost roughly **30×** versus LLM-judge approaches at comparable accuracy
- Produces per-token → per-span output, not just a binary verdict
...
- **Binary collapse** of RAGTruth's four annotated hallucination categories (Evident/Subtle × Conflict/Baseless-Info), discarding severity information that's already present in the data
...
1. **Turk-LettuceDetect (§2.7) already did the "translate RAGTruth, fine-tune" recipe for Turkish in September 2025.** Doing the identical recipe for Hindi would read as a direct copy to anyone on the panel familiar with the literature.
...
Both RAGTruth and PsiloQA rows reduce to the same shape — (context, question, answer, hallucination spans) — so the same `data_prep.py` label-alignment pipeline from Phase 1 (§ project brief) ingests both; only the JSON loader differs. That reuse is what keeps this feasible inside a sem-5 timeline. Three training runs, all still ModernBERT-base (150M parameters):
...
- It subsumes the old OOD stress-test idea (RAGTruth → PsiloQA is now the OOD test, and it's a real published dataset rather than a hand-built SQuAD slice) and gives the calibration work an actual comparison to report, instead of being a standalone, easily-dismissed bullet
...
RAGTruth already annotates *Evident Conflict / Subtle Conflict / Evident Baseless Info / Subtle Baseless Info* — the paper discards this and collapses to one bit before training even starts. Swap the classification head to 5-way (4 categories + supported) and show binary F1 doesn't suffer much, while the richer output supports a much more useful UI (a "confident contradiction" should look different from "subtle unsupported addition"). Low cost — the labels are already in the JSON you're loading — but a smaller, safer story than the combined-training study, worth doing only once that core result is solid.

**Note on Hindi/multilingual (from the original brief):** still a legitimate sem-6/7 idea, but not as a plain translation of RAGTruth (that's Turk-LettuceDetect's recipe). A more defensible angle would borrow **PsiloQA's (§2.9) automated, LLM-annotated data-construction pipeline** to build a cheap Hindi span-level test set, rather than relying on machine-translated RAGTruth.
...
- **But even with correct evidence, the answer can still contain unsupported claims** — a hallucination
...
- Use the timeline diagram from §4 — SelfCheckGPT → RAGTruth → Luna → ReDeEP → RAG-HAT → **LettuceDetect** → Turk-LettuceDetect / OOD-failure study → PsiloQA → RAGognizer
...
- Use the F1/advantage/disadvantage table from §5.8 and §2
